...
def constructCameras_waymo(frames_list, white_background, mapper = {},
    ...
    for idx, frame in enumerate(frames_list):
        ...
        #print("depth_max,depth_min:", np.max(depth_map),np.min(depth_map))
        ...
        # dir_path = "exp/debug-0"
        # if not os.path.exists(dir_path):
        #     os.makedirs(dir_path)
        # image_depth.save(os.path.join(dir_path, image_name + "_depth.png"))
        # depth_map_colored.save(os.path.join(dir_path, image_name + "_depth_colored.png"))
        # depth_map = frame.get('depth_map', None)
        # ------------------
        # load lidar_to_world
        # ------------------
...
def readWaymoInfo(path, model_path, white_background, eval, extension=".png", use_bg_gs=False, 
                  load_sky_mask=False, load_panoptic_mask=False, load_sam_mask=False, load_dynamic_mask=False, load_feat_map=False,
                  load_intrinsic = False, load_c2w = False,
                  start_time = 0, end_time = -1, read_freq = 1, num_pts = 5000, 
                  ):
    ...
    # mask
    stride = 0
    if stride != 0 :#TODO:所谓的新视角就是在这里划分
    ...
    if not os.path.exists(pts_path) or not load_lidar:
        ...
        pcd = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)))
    # storePly(ply_path, xyz, SH2RGB(shs) * 255)
    else:#TODO:用雷达初始化的时候，需要聚合，然后降采样吗？不然的话会不会太密集了
        # load lidar points
        ...
        for t in trange(0, len(lidar_filepaths), desc="loading lidar", dynamic_ncols=True):#
            ...
            lidar_origins = lidar_info[:, :3]#0,1,2
        ...
        # if save_occ_grid:#在给定的分辨率下指出哪些voxel被占据
        ...
        #         occ_grid = get_OccGrid(points, aabb, occ_voxel_size)#occ_voxel_size = 0.4
        ...
        # downsample points
        print("points before downsampling:",len(points))
        points,shs = GridSample3D(points,shs)
        print("points after downsampling:",len(points))
        
        #如果点比指定的初始化点数多，就随机采样初始化点数个点，设置为2000000
        ...
        print("number of points before store pcd:", len(points))
        ...
        full_pcd = pcd.fuse(bg_pcd) if use_bg_gs else pcd
        # load depth maps
        if load_depthmap:
    ...
    scene_info = SceneInfo(point_cloud=full_pcd,
